...
export class PaymentsService {
  ...
  async createMercadopagoPreference(data: ProductResearched[], tenancy: {apy_key: string, domain: string, company: string}): Promise<MercadopagoResponse> {
    ...
    const newPreference: CreateMercadopagoDto = {
      ...
      back_urls: {
        success: `https://${tenancy.domain}/payment/success`,
        failure: `https://${tenancy.domain}/payment/failure`,
        pending: `https://${tenancy.domain}/payment/pending`,
      },
      notification_url: `https://${envs.SERVER_URL}/api/v1/payments/mercadopago/confirm`,
    ...
    try {
      ...
      const response = await fetch("https://api.mercadopago.com/checkout/preferences", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${tenancy.apy_key}`,
